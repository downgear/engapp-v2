...
const Login = () => {
  ...
  useEffect(() => {
    ...
    handleGoogleCallback(loginToken)
      .then(() => navigate('/dashboard'))
      .catch((err) => {
        setError(err instanceof Error ? err.message : language === "vi" ? "Đăng nhập Google thất bại" : "Google login failed");
        setIsProcessingGoogle(false);
      });
  }, [searchParams, handleGoogleCallback, navigate, language]);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/5 via-background to-secondary/5">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground font-medium">
            {language === "vi" ? "Đang xử lý đăng nhập Google..." : "Processing Google login..."}
          </p>
        </div>
      </div>
    );
